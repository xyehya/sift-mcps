...
10. **Custody mutations are durable operations.** Portal Add/Seal binds case, sorted targets, actor, reason, scoped re-auth receipt, idempotency key, and the systemd `INVOCATION_ID` restart instance before Postgres records `REQUESTED` and durably blocks the gate. Local Immutable preparation pins and hashes every direct entry with `O_NOFOLLOW` before applying posture on those same descriptors. A new invocation atomically claims even `GATE_BLOCKED`; every advance, failure, and final commit compares the claimed runner as well as phase, so a stale process cannot mutate recovered work. A page-reloaded Portal resumes by public operation id only after fresh re-auth, server-side case/actor lookup, and strict stored-command reconstruction. Postgres separately validates that the new `reauth.evidence_seal_resume` audit receipt is successful, Portal-originated, original-user/case scoped, and bound exactly to the operation, then links it append-only without changing the original Seal digest. Custody tables remain inaccessible to the browser/authenticated DB role. A locked final transaction rejects any violation, derives the manifest version, preserves immutable sibling versions, records `canonical_event_v1`, completes the operation, and opens the gate exactly once. Later custody workflows reuse the typed repository/posture seams rather than creating parallel mutation paths. Their actions come from a closed server-selected enum and schema-v2 Evidence Object binding; Postgres derives scoped re-auth stages. A server-created recovery intent contains only object selection, actor, bounded reason, single-use audit capability ID, and idempotency key. Action-specific finalizers remain mandatory and cannot cross-call Add/Seal. Generalized begin and each action-specific finalizer acquire the per-case advisory transaction lock before their row locks; every finalizer owns and tests its internal row order. Operation-local `advance`/`fail` phase-CAS helpers are outside that case-first contract. No action seam adds MCP or browser database authority.
