...
9. **Evidence intake invalidates before it repairs.** Existing sealed files retain `+i`; a human VM-side addition can invalidate the case, after which `EvidenceGate` blocks every agent tool before dispatch. The pathless, DB-active-unsealed `--prepare` helper validates every direct entry but changes metadata only on eligible non-immutable incoming files, leaving sealed files untouched. Portal Rescan/Register/Seal is the only route back to custody `ok`; per-file Unseal is only for replacing or changing that sealed file.
