...
The CodeGuard rule files live at `.cursor/rules/codeguard-*.mdc` (one per rule).
...
2. List all rule files matching `.cursor/rules/codeguard-*.mdc`. The rule
...
3. For each rule, read its file. The YAML frontmatter declares applicability
   ...
   - Your own rule directory `.cursor/rules/` and any CodeGuard host
...
- If `.cursor/rules/` is missing or empty, stop and report that the CodeGuard
