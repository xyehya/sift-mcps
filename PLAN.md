...
```
1. SIFT VM SETUP      install.sh → install all mcp servers and requirements and gateway and portal and run checks with interactive results and outputs  → portal at https://<VM>:4508/portal/
...
5. AGENT STARTS       Hermes agent uses mcp to connect with service token → calls case_status → gets case_id,
...
Run this on SIFT VM before integration testing (must be automated through .sh installer)
...
### R0-7: Fix `make_ingest_tmpdir()` — use actual case dir (B8) - NOTE, this should resolve to a dedicated agent-specific subdirectory created under the active case dir and returns in response in case the AI Agents needs to run further outputs or inspection using bash tool.
...
### R5-2: Pre-Import Checklist for Any Code from Old obsolete repo
