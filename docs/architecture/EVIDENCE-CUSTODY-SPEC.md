...
- Retirement excludes a sealed Evidence Object through a new manifest while preserving history and protected bytes. Its transaction discharges only missing/content/identity causes bound to that exact retired object; append-only observations and every unrelated, ledger, conflict, or unsafe-entry cause remain. The synthetic persisted marker is removed only when no substantive cause or violated object remains. Physical purge is a separate high-ceremony workflow.
