...
- The canonical evidence root must itself be the external mount point. If that mount disappears and exposes a local underlay directory, reconciliation classifies storage as unavailable with reconnect-and-verify remediation; it does not reinterpret the writable underlay as read-write drift. A still-mounted expected source that becomes writable remains a posture violation, and a different source mounted at the root remains identity drift.
...
- `VERIFICATION_REQUIRED` never suppresses structural inventory truth: symlink, directory, nested, non-regular, and multi-link observations remain explicit unsafe findings and keep the gate blocked. External Add & Seal enumerates the pinned evidence root without following links before hashing and again at final posture verification. Selected, sealed, and ignored paths are required; retired historical paths are an optional allowlist because Retire is disposition-only and protected bytes may remain or may already be missing. Every present entry must be authorized, regular, single-link, and on the same source/mount. An unknown, unsafe, omitted-required, or raced entry prevents manifest, version, event, and verification-receipt commit.
...
The filesystem seam independently requires a no-follow enumeration of the pinned evidence root both
before hashing and immediately before the final receipt. Required current paths must be present;
retired paths are optional but no other names are allowed. Every selected name is reopened and its
device/inode/link/source/mount identity is compared with the long-lived pinned descriptor before and
after hashing, preventing selected-to-ignored swaps and pathname replacement. Retired presence never
selects or reactivates it and never enters the new receipt. Inventory classification retains
`UNSAFE_PENDING_ITEM` while storage verification is required, so the bootstrap predicate cannot
reinterpret that head as virgin.
