...
| `/etc/sift/custody/ed25519-private.pem` | `0640` | `root:sift-service` | installation-held Ed25519 custody signing authority; root-owned `0750` parent under root-owned `/etc` prevents service replacement; Gateway AppArmor receives only exact read traversal | yes |
