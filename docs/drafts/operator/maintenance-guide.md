...
- [`sift-tool-inventory.md`](../inventory/sift-tool-inventory.md) — live paths,
  services, Docker, modes.
- [`state-authority-map.md`](state-authority-map.md) — what the DB owns vs files/exports.
- [`reference-data-provenance.md`](reference-data-provenance.md) — RAG/FK/Hayabusa provenance.

Companion docs in this set (split for size, same batch):

- [`config-and-secrets.md`](config-and-secrets.md) — the full variable dictionary, every
  environment file / installer variable / DB-backed setting, and the
  **"what not to edit manually"** list.
- [`rag-and-search-maintenance.md`](rag-and-search-maintenance.md) — RAG, OpenSearch, and Hayabusa
...
| `/var/lib/sift/passwords/`, `/var/lib/sift/tokens/` (0700) | retired legacy hash (cleanup only), installer handoff | secret/config |
...
Full procedures are in [`rag-and-search-maintenance.md`](rag-and-search-maintenance.md) §1. RAG is
...
list with reasons is in [`config-and-secrets.md`](config-and-secrets.md) §"Do not hand-edit"
and includes:
