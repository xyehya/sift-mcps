...
| Sensitive human actions require fresh Supabase password re-auth | `routes._supabase_reverify`, `_record_reauth_event`; durable custody operation RPCs | The verifier binds the authenticated session identity, discards returned grant tokens, and records an action/object/operation-scoped `reauth_audit_event_id`. Service-only RPCs reject missing, reused, cross-case, cross-action, or wrong-actor receipts. No local password-derived HMAC or file-ledger authority remains. |
