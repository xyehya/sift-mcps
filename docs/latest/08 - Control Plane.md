...
Security-Definer RPCs include evidence detection, sealing, verification, closed drift classification, and durable operator dispositions. Portal routes do not invoke legacy one-shot ignore/retire mutations: Ignore, Delete Stray, and Retire begin a custody operation with an idempotency key and fresh re-authentication, block the gate, persist phase facts, and commit exactly once. Delete Stray is limited to readable pending Local Immutable files and records pinned pre-unlink identity/digest/size. The Gateway remains AppArmor write-denied; a fixed same-UID broker accepts only operation/runner identity, independently rebinds the Postgres operation and object, revalidates the file, and records an exact claim/completion receipt around unlink. Missing-file recovery without that claim fails closed. Retire preserves protected bytes and every prior version. Service-only RPC grants and forced RLS keep browser roles out of this authority path. Internal chain helpers append custody events and recompute the aggregate gate.
