...
- **RESOLUTION:** [RESOLVED 2026-06-02] Modified `_append_case_context` in gateway to selectively emit `_case` on orientation tools and `_case_ref` on others.
...
- **RESOLUTION:** [RESOLVED 2026-06-02] Updated `security.py` to print blocked exact tokens and safe equivalents. Modified `discovery.py` to return robust policy instructions for `get_tool_help('run_command')`.
...
- **RESOLUTION:** [RESOLVED 2026-06-02] Added scanner in `_execute_command` (generic.py) to detect parser failures in stdout/stderr, populating `warnings` and `agent_action` without overwriting the zero exit code.
...
- **RESOLUTION:** [RESOLVED 2026-06-02] See F-033.
- **RESOLUTION:** [RESOLVED 2026-06-02] Indirectly helped via F-033 warning extraction; full truncation implementation requires further core optimization.
...
- **RESOLUTION:** [RESOLVED 2026-06-02] Fixed relative path evidence registry checker in `case_manager.py`. Modified `record_finding` to ingest `audit_id` from `supporting_commands`.
...
- **RESOLUTION:** [RESOLVED 2026-06-02] Added `img_stat`, `fsstat`, `ewfinfo`, `ewfverify`, and `pinfo.py` to core catalog YAMLs.
