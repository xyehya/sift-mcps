...
| D4 | Active case (historical, superseded) | **Superseded by D32.** D4 is retained only as a historical pointer for older planning docs and Run logs. Do not cite D4 as the live active-case rule in new candidate docs or Build prompts; cite D32. |
...
| D32 | Active-case cutover model | PR03B / Batch B goes directly to the target active-case model: one deployment-scope active case in v1, selected by the operator in the portal and stored authoritatively in Supabase/Postgres `app.active_case_state`; other cases may exist but only the DB active case is operative. The Gateway propagates that DB active case to REST, MCP, core tools, and proxy policy. `SIFT_CASE_DIR`, `SIFT_CASES_ROOT`, `gateway.yaml case.dir`, and `~/.sift/active_case` are dropped/ignored as active-case authority and are **not** regenerated as compatibility exports. No historical data migration is part of this cutover. Existing memory images, disk images, case directories, and other forensic files may remain on disk and be referenced by DB case rows as artifact/data locations, but they do not decide the active case. Stale env/config/pointer values must be negative-tested. Locked by operator 2026-06-07 (Run 31), with D4 demoted on Run 32. |
| D33 | Add-on backend credential references | D22A stores only non-secret backend connection metadata and structured credential references in `app.mcp_backends`; usable backend secret material does not live in repo files, migration files, DB rows, fixtures, snapshots, logs, audit payloads, or generated docs. The D22A implementation resolves `env` references from the Gateway process environment at backend-load time (`bearer_token_env`, `tls_cert_env`, `env_refs`) and rejects raw secret-bearing fields at registration. Supabase Vault is an allowed future reference source only after a separately scoped secret-lifecycle design; D22A does not enable Vault, write Vault secrets, or accept raw backend secrets through the portal. Locked by delegated operator decision 2026-06-07 (Run 38, resolving F-14), grounded in doc 22 §5.3/§13, Supabase Vault docs, and the existing Gateway `auth.*_env` config pattern. |
| D34 | Add-on backend activation model | D22A uses restart-to-apply for FastMCP `/mcp` catalog exposure. The DB registry is authoritative immediately for REST/portal metadata, but a register/enable/disable/unregister change is marked `pending_apply` until the Gateway process restarts and rebuilds the aggregate FastMCP server from `app.mcp_backends`. `POST /api/backends/reload` refreshes registry/runtime metadata and reports `restart_required`; it does not live-remount or mutate the active FastMCP provider tree. This avoids mid-session namespace drift and preserves the D24 SIFT-owned policy middleware ordering. A later zero-restart dynamic remount may be designed only as a new scoped fork. Locked by delegated operator decision 2026-06-07 (Run 38, resolving F-15), grounded in FastMCP 3.4.2 source/API and doc 22 §7/§13. |
