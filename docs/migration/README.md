...
- [19_pr03_unified_supabase_jwt_identity.md](19_pr03_unified_supabase_jwt_identity.md) - historical **PR03A / Batch A** candidate/log: Supabase JWT validation for REST and FastMCP `/mcp`, shared Gateway principal resolution, portal Supabase login/session, agent/service JWT issuance, DB-backed tool scopes, B-10 list/call authorization, B-14 resolver cleanup, tests, VM gates, and archived build prompt.
- [20_portal_dashboard_inventory.md](20_portal_dashboard_inventory.md) - normalized portal/dashboard workflow and API inventory from the separate read-only portal scan. Reference inventory for portal/data-authority turnover; not an implementation candidate.
- [21_pr03b_active_case_db_authority.md](21_pr03b_active_case_db_authority.md) - historical **PR03B / Batch B** candidate/log: Postgres `active_case_state` authority, portal case API turnover, Gateway REST/MCP propagation, B-11 proxy active-case handling, stale env/config/pointer negative tests, no historical data migration, and archived build prompt.
- [22_d22a_mcp_backends_registry.md](22_d22a_mcp_backends_registry.md) - historical **D22A / Batch H** candidate/log: `app.mcp_backends` control-plane registry (no-raw-secret credential references, health/manifest cache, RLS/D12), DB-authoritative Gateway backend loader (`gateway.yaml` backends removed as authority), portal/REST turnover, FastMCP mount/policy ordering preserved, B-13 resolution, F-11 resolution at Land, post-Land Run 40 hardening (minimal stdio env, DB connection-shape checks, unregister), VM test plan, scope fence, and archived Build prompt. F-14/F-15 resolved to D33/D34; F-11 resolved and B-13 done at Land.
...
6. Update [MIGRATION_STATE.md](MIGRATION_STATE.md) at the end with files inspected, decisions, open questions, and the next run.

Read [MIGRATION_STATE.md](MIGRATION_STATE.md) for the live Current Objective,
latest run, and next action. Do not use this index as a current-state handoff.
