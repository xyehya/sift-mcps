...
- [19_pr03_unified_supabase_jwt_identity.md](19_pr03_unified_supabase_jwt_identity.md) - Build-ready **PR03A / Batch A** implementation candidate: Supabase JWT validation for REST and FastMCP `/mcp`, shared Gateway principal resolution, portal Supabase login/session, agent/service JWT issuance, DB-backed tool scopes, B-10 list/call authorization, B-14 resolver cleanup, tests, VM gates, and ready-to-copy build prompt.
...
recommended run is the **Build-stage PR03A / Batch A** implementation from
[19_pr03_unified_supabase_jwt_identity.md](19_pr03_unified_supabase_jwt_identity.md):
unified Supabase JWT authentication for REST and FastMCP `/mcp`, shared
operator/agent/service principal resolution, portal Supabase login/session,
agent/service JWT issuance, and DB-backed tool authorization. Keep D22/F-11
(`mcp_backends` control-plane registry) and ID-4/ID-5 active-case propagation
separate unless a candidate doc explicitly batches them.
