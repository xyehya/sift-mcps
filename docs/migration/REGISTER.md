...
| B-14 | Remove duplicate MCP token resolution on the D27b FastMCP path. Review S-3 found the raw ASGI wrapper and `SiftTokenVerifier` both resolve identity; keep correctness but avoid the double registry lookup by sharing the wrapper identity or rationalizing the auth layers. | Run 24 (D27b review S-3) | OPEN | auth cleanup / ID-6 |
