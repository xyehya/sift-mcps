...
| B-10 | Per-principal **tool authorization** under the D30 Supabase JWT model: let a human/agent/service principal restrict which tools it may list/call (for benchmarking, least privilege, and testing tool updates against a controlled subset). Existing PR02 `app.mcp_token_scopes` + `Identity.tool_scopes` can inform the bridge, but the target enforcement resolves scopes from JWT subject → app principal/membership/scope rows. Decide the scope grammar (e.g. `tool:<name>`, `namespace:<ns>`, `capability:<name>`) and enforce **SIFT-side** in the gateway `on_call_tool` middleware (reject before `call_next`) AND filter `on_list_tools` (list/exec consistency) — keep it SIFT-owned per D24/D30, do not delegate final policy to FastMCP `require_scopes`. | F-9 / Run 22 / Run 26 D30 | OPEN | PR03A/ID-3 auth or ID-6 auth cleanup |
