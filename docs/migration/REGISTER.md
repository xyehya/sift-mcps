...
| B-5 | `opensearch_case_detections_resource` ignores its `case_id` path param (returns active-case detections regardless; D4 single-active-case masks the gap). Scope the query by `case_id` or drop the path parameter so the URI does not promise scoping it cannot deliver. D27b confirmed this requires `packages/opensearch-mcp/**`, outside the gateway scope fence, so it remains deferred. | Run 21 (D27a review, S2) / Run 24 | OPEN | OpenSearch core/hardening phase |
