...
- LIVE STATE: VM up, services active, `/health` 18 tools (wintriage not yet registered). Evidence
  `Rocba-Memory.raw` + `rocba-cdrive.e01` present in the active case but `detected`/`unsealed`, `root`-owned, not
  immutable (B-MVP-048 pre-seal state). **Next:** relocate the existing 5.6GB to the configured dir (avoid a 5.9GB
  re-download) → operator launches + portal-registers wintriage (B-MVP-040(d)/B-MVP-038 on fresh install); then
  evidence seal + forensic re-verify.
