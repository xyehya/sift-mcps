...
| B-MVP-042 | Backlog | OPEN | Memory-ingest hostname AUTO-DERIVATION (from B-MVP-037; the discoverer `host_discovery.py` has no memory source, so `format=memory` hard-requires `hostname`). APPROACH PROVEN read-only on the live VM 2026-06-15 (all 3 probes → `SRL-FORGE`, matching the manual ingest; ~1.6–2.0s on the 19GB image, warm symbol cache): vol3 `windows.registry.printkey` on `ControlSet001\Control\ComputerName\{ActiveComputerName,ComputerName}` (`-r json`, strip REG_SZ quotes) PRIMARY, `windows.envars` COMPUTERNAME majority SECONDARY. Implement as a preflight in the `format=memory` path: derive when `hostname` absent → canonicalize via `host_dictionary.resolve()` (mirrors `hostname.py`'s mounted-volume ControlSet logic) → keep `hostname=` override and the current error only as last resort. Full command/JSON-shape/parser recipe + gotchas: `docs/B-MVP-042-memory-hostname-autoderive.md`. Also surface the requirement in the tool schema + the cosmetic double `case-case-` index-prefix quirk. | BATCH-LV1 |
