...
| B-MVP-044 | Backlog | OPEN | install.sh docker-group bootstrap UX (fresh-install run): when the operator's current session lacks the docker group, install.sh's docker-unreachable message prints `sudo usermod -aG docker` but NOT the session-refresh step, so the operator is stuck until they discover `newgrp docker` / a fresh login. Reword the guidance to include `newgrp docker` (or "open a new shell / re-login") so the same session can proceed. | BATCH-LV1 |
...
| B-MVP-046 | Backlog | OPEN | AppArmor enforce-flip not automated (fresh-install run): install.sh installs the SIFT profiles in COMPLAIN mode by default (install.sh:2916; verified live 77 enforce / 9 complain incl `/opt/sift-mcps/.venv/bin/python`). The hardened end-state (RUN-3 / B-MVP-026 proved enforce) requires a DELIBERATE post-validation `aa-enforce` flip that is NOT part of the install. Document the flip as the final hardening step and/or add an opt-in `install.sh --apparmor-enforce` (or a `harden.sh`) so the operator reaches the enforce posture intentionally. | BATCH-HR3 |
| B-MVP-047 | Backlog | OPEN | wintriage baseline provisioning (fresh-install run): `setup-addon.sh` wintriage download (`download_databases`, ~5.9GB) did NOT persist to the `SIFT_WINDOWS_TRIAGE_DB_DIR` passed at the prompt — after the run `/opt/sift-mcps/windows-triage-db` was EMPTY (download progressed in the log but landed nowhere useful). Note: that run was from the operator clone (B-MVP-040(a) fallback path); re-test from `/opt/sift-mcps`. Verify `download_databases` honors `SIFT_WINDOWS_TRIAGE_DB_DIR`, writes a writable location, and document the canonical baseline dir the gateway (`sift-service`) reads (prior VM used `/var/lib/sift/windows-triage`; setup-addon default is `/var/lib/sift/.sift/windows-triage-db`). | BATCH-LV1 |
| B-MVP-048 | Backlog | OPEN | Evidence intake friction (fresh-install run): the active case evidence dir `/cases/<case>/evidence` is `sift-service`-owned (`drwxr-xr-x+`) and NOT operator-writable — so the operator cannot `cp` evidence in directly (arch: "evidence bytes are copied only by the operator"). Document/streamline the supported intake (operator `sudo cp` + `chown sift-service` into the case evidence dir, OR a portal evidence-import that copies from a source path), so the "copy evidence into the active case" step in the install epilogue is actionable. | BATCH-LV1 |
