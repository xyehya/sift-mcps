...
| B-MVP-042 | Backlog | OPEN | UX (from B-MVP-037): `opensearch_ingest format=memory` silently requires a `hostname` arg — omitting it fails fast with a clear message but the requirement isn't discoverable. Surface it in the tool's `when_to_use`/input schema (or make `hostname` conditionally-required in the schema). Also a cosmetic double `case-case-` index-prefix quirk on the vol index name. Minor. | BATCH-LV1 |
