...
Status: DONE (merged to main; tests green on root). Implemented the design-doc recommendation (option c): the shared
add-on `deprecated_aliases` feature — served-but-undeclared tools that 500'd the portal Start flow against the gateway's
`served ⊆ manifest` invariant — is removed entirely (it was DEFINED BUT UNUSED in all add-ons).

- Removed the `deprecated_aliases` field from the `ToolDef` contract in ALL THREE add-ons that carried the shared
  contract (opensearch-mcp, opencti-mcp, AND windows-triage-mcp — the agent found the third), plus the alias-serving
  loops + alias meta (`deprecated_alias_of`/`canonical_name`) in the registries and the dead `deprecated_aliases=[]`
  cutover marker. `grep -rn deprecated_alias packages/` is now CLEAN.
- NO golden changed (the `"deprecated": true` entries in the opensearch/opencti goldens are canonical
  tool-vs-resource deprecations in `_TOOL_META`/`TOOL_CATALOG_META`, unrelated to the removed alias feature).
- Authoring docs: `docs/add-ons/author-guide.md` §6.7 now documents the `served ⊆ manifest` strict-on-start invariant
  and the SAFE rename path (add the new tool name to the manifest + bump `manifest_sha256` + re-register). Design doc
  status flipped to "DECIDED — option (c) implemented".
- Regression test guarding the invariant the feature violated:
  `test_f1_opensearch_backend_registry.py::test_started_backend_serving_undeclared_tool_is_rejected` — a started
  backend whose `tools/list` includes a namespaced-but-undeclared tool is rejected by `_build_tool_map`.
- Tests (root env): opensearch 1060, opencti 16, windows-triage 25, gateway 533 (incl. the new test). validators +
...
(migration applied, core/gateway/route synced to `/opt`, portal UI rebuilt+synced, gateway restarted, `/health` 24 ok)
and ran the full lifecycle with the operator driving the portal while the agent validated over MCP + ssh.

- **Live prereqs verified:** `/usr/bin/python3.12` has `cap_linux_immutable=ep`; sift-service set+cleared `+i`
  in-process on `/cases` (ext4, supports `+i`).
- **INTAKE-CHOWN REQUIREMENT (found live):** sift-service CANNOT set `+i` on a `root:root` file (it has
  CAP_LINUX_IMMUTABLE but not CAP_FOWNER, and isn't owner) → `Operation not permitted`. So evidence must be
  `sift-service`-owned for in-process `+i`. This is the documented intake model (operator copies THEN chowns to the
  service user); it keeps the design helper-free (operator's own sudo does the one-time intake chown; gateway manages
  `+i`/`-i` in-process thereafter). Hardened the case's 2 files = chown sift-service + `+i` via the DEPLOYED code.
- **LIFECYCLE PROVEN LIVE:** seal → both files `+i` + case `sealed` + gate OPEN (tools work). Operator clicks **Unseal**
  (re-auth) → that file's `+i` CLEARED on disk, item → `detected`/`unsealed`, case aggregate `unsealed`, and EVERY agent
  MCP tool blocked (`evidence_chain_unsealed`) — the intended control. Operator **re-seals** via the Seal Manifest →
  `+i` restored, case `sealed` (manifest v2), gate REOPENED (wintriage/opensearch usable again). Verified both unseal
  paths (drive + memory) and re-seal.
- **2 FIXES during the test** (`78118a8`): (1) UI Unseal button was gated on the CASE-aggregate seal_status, so
  unsealing one item hid the button on every still-sealed item — now gated PER-ITEM on `chainStatus.ok` (+ regression
  test). (2) `app.evidence_unseal` set status=`registered`, but the portal Seal-Manifest (re-seal source) only includes
  `detected` items → unsealed items couldn't be re-sealed; now sets `detected` (metadata columns persist; gate still
  blocks). gw + frontend unseal tests green (5/5 each).
- **FOLLOW-UPS (non-blocking):** document the intake chown-to-service-user step (evidence intake) and reword the
  `harden_sealed_evidence` failure message to name ownership (root:root → not immutable in-process) as the remedy. The
  `/security-review` of the unseal surface was CLEAN (prior entry).
...
- **B-MVP-048 — seal-flow FS hardening (code DONE on main; live deploy + install provisioning operator-gated):**
  ROOT CAUSE (agent, confirmed): the live DB-authoritative seal `EvidenceAuthorityService.seal()`
  (`portal_services.py`) only calls `app.evidence_seal` (logical seal) and NEVER invoked any filesystem
  hardening — the `_set_immutable` code inside `evidence_chain.seal_manifest()` is DEAD on the live path. That is
  why the operator-`sudo cp`'d evidence stayed `root:root 0644` / mutable after sealing. The gateway runs as
  non-root `sift-service`; the venv interpreter carries `CAP_LINUX_IMMUTABLE` (so `+i` works in-process) but the
  unit has no `CAP_CHOWN` (so re-owning a root:root file needs a privileged helper). FIX (`ccb4336` + sec-review
  hardening `330dfb6`): new `harden_sealed_evidence()` in sift-core (resolves strictly inside `<case>/evidence/`,
  rejects symlinks/traversal, sets `+i`, fails CLOSED if immutability can't be achieved), wired into `seal()`
  BEFORE the DB write (failure → `evidence_immutability_failed` 500, so a logical seal is never recorded for
  un-hardened bytes); plus a narrow root helper `scripts/sift-seal-evidence` (chown→service-user + `chattr +i`).
  `/security-review` found ONE HIGH (the helper's `/evidence/` check was an unanchored substring with no case
  binding → with the sudoers grant a compromised gateway could chown/`+i` an arbitrary inode, hardlink-assisted);
  FIXED in `330dfb6` — helper now requires `--case-dir` and anchors `--path` strictly under `<case-dir>/evidence/`
  + rejects hardlinks (`st_nlink!=1`). sift-core evidence suite 84 passed; gateway evidence/policy 61 passed.
  **STILL OPEN (operator-gated, NOT done):** (1) `install.sh` must install the helper to `/usr/local/sbin/` and
  add a command-specific NOPASSWD sudoers drop-in for the service user (mirroring `setup-ingest-mount-sudoers.sh`
  / the run-command systemd-scope helper) — NOT yet written (operator asked to review the grant first). (2) LIVE
  DEPLOY is gated: before deploying, verify the live `/opt/.venv` interpreter has `CAP_LINUX_IMMUTABLE` and the
  `/cases` filesystem supports `+i`, else the new fail-closed seal would 500 future seals. (3) The CURRENT case
  evidence (`root:root`, mutable) can be hardened now via the operator running chown+`chattr +i` (or the helper
  once provisioned).

### 2026-06-16 - Parallel agent-team batch: install polish (043/045/046) + setup-addon UX (049/050/051) + B-MVP-052 design — 3 units merged to main

Status: DONE (host code + validators; live re-test of 043/046 carries forward). Orchestrated 4 parallel agent teams on
manual worktrees off `main` HEAD (NOT Agent isolation:worktree — that branches off stale origin/main). Zero file
overlap; each merged `--no-ff` with no conflicts; authoritative `bash -n`/validators re-run on main after each merge.

- **B-MVP-043 DONE** (`2e1f6da`, merge `8a918c4`): `install.sh apply_db_migrations()` now reads
  `supabase_migrations.schema_migrations` up front and SKIPS any migration `supabase start` already applied (version =
  filename timestamp prefix, the CLI convention). Fresh DB / non-CLI DB → ledger absent → empty applied-set → every
  migration still runs (fresh-install behaviour unchanged); unreachable-DB still dies on the first real file. Kills the
  `already exists` / `cannot drop columns from view` re-apply noise at a single point (no migration-file edits).
- **B-MVP-045 DONE** (`2e1f6da` + `8d8ff30`): install.sh now logs `configured execute.runtime_user=… (restart applies
  after ACL changes)`; AND the actual misleading source string in `scripts/setup-agent-runtime.sh:178-179` ("Set
  execute.runtime_user … and restart") reworded to an ensure/verify phrasing (correct standalone and in the install
  flow). [The agent flagged the source string was outside its install.sh scope-fence; orchestrator fixed it.]
- **B-MVP-046 DONE** (`2e1f6da`): AppArmor default stays COMPLAIN; the proven ENFORCE posture (RUN-3/B-MVP-026) is now
  reachable deliberately via `install.sh --apparmor-enforce` (or `SIFT_APPARMOR_ENFORCE=1`) and a new self-contained
  `harden.sh` (`aa-enforce` the 2 SIFT profiles, `--complain` to revert). New `_apparmor_load_profile` helper.
- **B-MVP-049/050/051 DONE** (`434b26e`, merge `672dd89`; `scripts/setup-addon.sh` only): (049) staged root now resolved
  from `SIFT_MCPS_INSTALL_ROOT` (default `/opt/sift-mcps`, env-overridable) independent of where the script lives, with
  fail-fast `die` if the runtime venv is absent; all 4 payload manifest paths repointed to `$SIFT_MCPS_ROOT`. (050)
  emits a labeled portal-form-ready block (Transport/Name/Manifest/Command/Args one-per-line+JSON/Env-refs) derived from
  the same payload globals as the JSON. (051) wintriage prompt default = the add-on config default
  `/var/lib/sift/windows-triage`; when the chosen dir == default → emit NO env-ref (registers out of the box); when
  overridden → keep env-ref + queue an operator note to set it in a gateway EnvironmentFile + restart. Dry-run smoke
  exercised all 4 paths. (Hard-codes the wintriage config default with a comment — single coupling point.)
- **B-MVP-052 design doc** (`56b42cc`, merge `cc77620`): `docs/design/B-MVP-052-deprecated-aliases.md` recommends
  **option (c) — remove `deprecated_aliases` from the shared add-on contract** (defined but UNUSED in both opensearch &
  opencti today; eliminates the guaranteed future portal-Start 500; preserves served⊆manifest). Option (b) rejected on
  security grounds (an exempted alias would carry no required_scopes/case_scoped meta → scope/case-isolation bypass +
  voids manifest_sha256 pin). DECISION PENDING operator; stays OPEN until applied. Doc also suggests a started-backend
...
| B-MVP-045 | Backlog | DONE | install.sh misleading message (fresh-install run): the run_command user-isolation step prints `Set execute.runtime_user: "agent_runtime" in gateway.yaml and restart …` as if it's a manual operator TODO, but install.sh ALREADY sets it programmatically (install.sh:2068-2069; verified live: gateway.yaml `runtime_user: "agent_runtime"`). Reword to "configured `execute.runtime_user=agent_runtime` (restart applies after ACL changes)" so it doesn't read as an unfulfilled action. | BATCH-LV1 |
| B-MVP-046 | Backlog | DONE | AppArmor enforce-flip not automated (fresh-install run): install.sh installs the SIFT profiles in COMPLAIN mode by default (install.sh:2916; verified live 77 enforce / 9 complain incl `/opt/sift-mcps/.venv/bin/python`). NOTE: enforce mode is KNOWN-GOOD — RUN-3 / B-MVP-026 already PROVED `run_command` under FULL security (Landlock + seccomp=kill + AppArmor=ENFORCE as agent_runtime) BEFORE this run — so this is purely an install-DEFAULT + automation gap, not a functionality question. The hardened end-state needs a DELIBERATE post-validation `aa-enforce` flip that is NOT part of the install. Document the flip as the final hardening step and/or add an opt-in `install.sh --apparmor-enforce` (or `harden.sh`) so the operator reaches the proven enforce posture intentionally. | BATCH-HR3 |
