...
| B-MVP-012 | Needs input | OPEN | BLOCKED-ON-DECISION 2026-06-12: HR3 proved rotation is infeasible on the Supabase CLI local stack (v2.105.0 bakes the demo GOTRUE_JWT_SECRET/keys/DB password with no config override; partial rotation breaks db/auth/rest/kong). Options: (a) accept lab posture - stack is loopback-only, gateway is the only exposure, manual external-project rotation documented in config-and-secrets.md section 5.1; (b) replace the CLI stack with a self-managed compose that owns GOTRUE_JWT_SECRET + generated keys + non-default password (significant new batch). | Operator |
...
| B-MVP-017 | Needs input | OPEN | Retire the remaining legacy file-HMAC plane: case-dashboard local re-auth bridge (`_sync_local_reauth_password` + file-authority challenges for commit/evidence/report), the legacy `sift_session` cookie verification middleware (nothing mints it anymore), and sift-core `verification.py` writer funcs + `backup_ops.py` ledger-copy that only that bridge consumes. Decide: retire now (DB/Supabase authority is live) or keep for file-authority deployments. | Operator / future batch |
