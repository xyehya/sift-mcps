...
| B-MVP-040 | Backlog | DONE | 2026-06-15 (`2be4ac0`, merge `f80eb7c`; team lv1-cleanup): host hardening (a/b/c) of `setup-addon.sh stage_runtime_command` landed — (a) staged-root check (accept the console command only when `$venv_bin == ${SIFT_MCPS_INSTALL_ROOT:-/opt/sift-mcps}/.venv/bin`, else loud fallback+remediation, fixing the operator-checkout silent-RED path); (b) fail-fast `die` on empty `SIFT_MCPS_ROOT`/`PYTHON_BIN`; (c) collapsed the always-equal doubled positional to one `<console-script>` (all 4 callers updated; `setup_custom` untouched); + empty-array-safe fallback loop. `bash -n` clean; `die` confirmed available (sourced install.sh). **(d) STILL PENDING a LIVE fresh-install test** (could reveal `uv sync --inexact` prunes the `full`-synced core backends → would reopen): pairs with the operator's fresh VM, alongside the B-MVP-034 fresh-register confirm and B-MVP-033. | BATCH-LV1 |
