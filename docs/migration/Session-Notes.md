...
| B-MVP-011 | Needs input | OPEN | DECIDED 2026-06-12: drop both legacy file-mode fallbacks. Execution split across the wave-3 worktrees: sift-core file-HMAC verification ledger retirement in BATCH-HR3; examiner.json PBKDF2 auth fallback removal in BATCH-PT1 (it lives in case-dashboard auth code PT1 owns). Supabase/DB becomes the only auth/verification path with clean errors when unavailable. | BATCH-HR3 / BATCH-PT1 |
...
| B-MVP-014 | Needs input | OPEN | DECIDED 2026-06-12: HR3 installs auditd with a minimal forensic-workstation ruleset (watch /var/lib/sift/.sift secrets/config, /cases evidence, /opt/sift-mcps binaries, identity changes), persistent across reboots with rotation configured. | BATCH-HR3 |
| B-MVP-015 | Needs input | OPEN | DECIDED 2026-06-12: canonicalize `BAAI/bge-base-en-v1.5` (matches the live 26,586-chunk corpus; no re-embedding): fix the allowlist, pin the model revision per B-MVP-004, and set explicit `HF_HOME` under the service home in gateway/worker env. Implementation in BATCH-HR3. | BATCH-HR3 |
