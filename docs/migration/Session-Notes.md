...
| B-MVP-036 | Backlog | DONE | 2026-06-15 (lv1/opensearch `530b1b4`): did NOT reproduce on current main — `opensearch_count` and all 11 case-scoped tools already accept the injected `case_dir`; the LV1 failure was a STALE staged tree on the VM (version drift). Added manifest-driven regression guard (`TestCaseDirArgInjectionInvariant` over every tool with `case_dir` in `safe_case_argument_names`, + `TestCaseDirKwargNoRaiseLive`) that would have caught it and auto-covers future tools; also fixed a pre-existing `_INJECTED_CASE_DIR` ContextVar test-pollution leak (1027→1044 passed). LV2 fresh install confirms on the live tree. | BATCH-LV1 |
