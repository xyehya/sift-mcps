...
// view, full functional parity). ONE primary scroll owner. Header (H1 per the
// §A typography bar) → filter bar (priority · status · New TODO) → examiner-only
// create form → table of TodoRows (inline edit · single-click status toggle ·
// overflow edit/delete · related-finding navigation). RBAC: write actions are
// examiner-only (canWrite). Empty states for no-todos and no-filter-match.
//
// Design-polish (DESIGN-POLISH-CONTRACT §C TODOs): dominant H1 (no static
// "(N of N)" chrome), a live filter-reactive "N shown" only while filtered,
// no "X open · Y completed" decoration, "+ New TODO" recentred off the border,
// severity carried by token colour + Title-case label (no shape glyph), row
// actions collapsed to one primary affordance + a ⋯ menu.
//
...
export function TodosTab() {
  ...
  const { todos, setTodos, setActiveTab, setSelectedFindingId, isLoading, addToast, user } =
    useStoreSlice((state) => ({
      todos: state.todos,
  ...
  // Live, filter-reactive count only — never a static "(N of N)" decoration (§B3).
    ...
    return (
      <div className="h-full space-y-4 overflow-y-auto bg-bg-base p-5">
        <div className="border-b border-border-faint pb-3">
          <h1 className="font-display text-[22px] font-bold leading-none tracking-[-0.4px] text-foreground">TODOs</h1>
        </div>
  ...
  return (
    <TooltipProvider delayDuration={150}>
      ...
      >
        <div className="shrink-0 space-y-4 border-b border-border-faint pb-4">
