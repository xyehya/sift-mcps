...
# PMI1: OpenSearch 3.5 runs with the security plugin DISABLED and :9200 bound to
# loopback (the security boundary). Plain http, no TLS, no real auth.
...
# Security is disabled, so no real password is needed. The opensearch-mcp client
# still requires user/password keys in the config, so write placeholder admin
# credentials over http; the security-disabled server ignores them.
mkdir -p "$SIFT_DIR"
...
# --- 3. Start OpenSearch ---
# Security plugin disabled (PMI1): no OPENSEARCH_INITIAL_ADMIN_PASSWORD needed.
...
for i in $(seq 1 90); do
    HEALTH=$(curl -fsS "$OS_URL/_cluster/health" 2>/dev/null || true)
...
# Clean up test index
curl -fsS -X DELETE "$OS_URL/case-test-evtx-smoketest" >/dev/null 2>&1
...
# Security plugin disabled (PMI1): plain http on loopback, no auth.
url = '$OS_URL'
