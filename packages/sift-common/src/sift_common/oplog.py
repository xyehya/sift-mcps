...
Structured JSON logging to stderr and optionally to ~/.sift/logs/.
...
def setup_logging(
    ...
    *,
    ...
    Args:
        ...
        level: Logging level.
        json_format: Use JSON formatting. If None, checks SIFT_LOG_FORMAT env
            var (default: "json"). Set to "text" for plain text.
        log_to_file: Write to ~/.sift/logs/{service_name}.jsonl. If None,
            checks SIFT_LOG_FILE env var (default: "true").
    """
    if json_format is None:
        json_format = os.environ.get("SIFT_LOG_FORMAT", "json").lower() != "text"
    if log_to_file is None:
        log_to_file = os.environ.get("SIFT_LOG_FILE", "true").lower() in (
            "true",
            "1",
            "yes",
        )
    ...
    # Optionally log to ~/.sift/logs/
    if log_to_file:
        try:
            log_dir = Path.home() / ".sift" / "logs"
            ...
            pkg_logger.warning(
                "Failed to set up file logging to ~/.sift/logs/: %s: %s",
                type(exc).__name__,
                exc,
            )
