...
def _find_catalog_dir() -> Path:
    ...
    # Relative to this file: src/sift_core/execute/catalog.py → ../../../data/catalog/
    source = Path(__file__).resolve().parent.parent.parent.parent / "data" / "catalog"
    if source.is_dir():
        _CATALOG_DIR = source
        return source

    # Wheel layout: force-included data lives under sift_core/data/catalog.
    packaged = Path(__file__).resolve().parent.parent / "data" / "catalog"
    if packaged.is_dir():
        _CATALOG_DIR = packaged
        return packaged

    raise FileNotFoundError("Cannot find sift-core execute catalog directory.")
