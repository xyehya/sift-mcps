...
- [ ] Commit/stash the current working-tree changes on `main` (there are uncommitted deletions + untracked docs).
- [ ] Tag the pre-revamp baseline: `git tag pre-revamp-v0 && git push --tags`.
- [ ] Create the revamp branch: `git switch -c revamp/spg-v1`.
- [ ] Add a worktree so `main` and the revamp can be checked out at once for side-by-side live testing: `git worktree add ../sift-mcps-main main`.
- [ ] `revamp-tasks.md`, `revamp-plan.html`, and `docs/revamp/*` live on `revamp/spg-v1`.
...
At the **end** of every session, append an entry to the **Session Log** (§7). Keep it short and mechanical:
...
- [ ] **6.1 Namespace the four add-ons:** `opensearch_*`, `cti_*`, `kb_*` (forensic-rag), `wintriage_*`. Give each a `sift-backend.json` manifest. Rename `case_host_fix`→`opensearch_host_fix` (stays in opensearch, F-D).
  - *Test:* each add-on passes the conformance probe; full e2e on fresh VM with all four add-ons enabled; full ROCBA regression case.
...
## 6 · Cross-cutting invariants to preserve (check at every gate)
