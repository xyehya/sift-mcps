...
- [~] **6.2 Namespace + migrate the four add-ons** following the **§5 migration playbook** (order: rag → windows-triage → opencti → opensearch): `kb_*`, `wintriage_*`, `cti_*`, `opensearch_*`. Give each a `sift-backend.json` manifest **now also declaring** the `category`/`recommended_phase`/`health` metadata moved out of core in 6.1. Rename `case_host_fix`→`opensearch_host_fix` (stays in opensearch, F-D). **3 of 4 done (Session 24): rag, windows-triage, opencti. OpenSearch remains (the big one).**
  ...
  - *requires[] choice (all 3):* declared `requires: []` — each backend self-reports availability via its health tool and degrades gracefully (no hard prereq to gate on; fail-closed `evaluate_requirement` makes a wrong `env:`/`host:port` entry worse than none). Revisit for opensearch (real OpenSearch service dependency → likely a genuine `requires` entry).
  - *NOT DONE — opensearch (`opensearch_*`):* the big one. ~20 `idx_*` tools (`@server.tool` decorators, FastMCP fn names) + `case_host_fix`→`opensearch_host_fix` (F-D) + `idx_install_pipelines` opting out of agent view via manifest `hidden_from_agent` (replaces the old hardcoded `_AGENT_FILTERED_TOOLS` entry). 973 opensearch tests reference `idx_*` names → expect heavy test fallout. Inventory started: registrations at `server.py:711+` (`idx_search`, `idx_count`, `idx_aggregate`, `idx_get_event`, `idx_timeline`, `idx_field_values`, `idx_status`, `idx_shard_status`, `idx_install_pipelines`, `idx_case_summary`, `idx_inspect_container`, `idx_ingest`, `idx_ingest_status`, `idx_enrich_intel`, `idx_enrich_triage`, `idx_list_detections`, `case_host_fix`, + health/status). Decide naming: keep `idx_*` and just add an `opensearch` namespace prefix → `opensearch_idx_*`? Or rename `idx_*`→`opensearch_*`? **Open question for next session** (manifest `namespace` must be the literal tool prefix the gateway enforces — every advertised tool must start with `<namespace>_`).
